...
//   PIX_AMOUNT       -> valor da entrada (opcional, padrão 10)
...
exports.handler = async (event) => {
  ...
  const AMOUNT   = Number(process.env.PIX_AMOUNT || '10');
  ...
  const DEADLINE = Date.parse('2026-07-05T20:00:00Z');
  ...
  try {
    ...
    const chkRes = await fetch(
      SB_URL + '/rest/v1/palpites_noruega?nome_key=eq.' + encodeURIComponent(nome_key) + '&select=pago,mp_payment_id',
    ...
    const mpRes = await fetch('https://api.mercadopago.com/v1/payments', {
      ...
      body: JSON.stringify({
        ...
        description: 'Bolao Brasil x Noruega - ' + nome,
    ...
    const upRes = await fetch(SB_URL + '/rest/v1/palpites_noruega?on_conflict=nome_key', {
      method: 'POST',
      headers: {
        'apikey': SB_KEY,
